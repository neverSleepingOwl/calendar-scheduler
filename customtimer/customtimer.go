...
//Init - fill timer with initial values
func Init() (timer * CTimer) {
	return &CTimer{make(chan bool), 0, false, sync.Mutex{}, 0}
}

//Set -  set timer's delay if timer isn't running
...
//Add - add some time to timers delay (if timer is set to 10 and before it expires we'll add 6 seconds
...
//Reset reset timer with a given period of time. whenever it's running or not.
func (t * CTimer) Reset(delay uint32){
	if t.expiresFromNow() > 0 {
		t.mu.Lock()
		defer t.mu.Unlock()
		t.delay = delay
